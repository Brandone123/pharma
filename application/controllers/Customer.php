...
class Customer extends CI_Controller {
   ...
   public function Wholesale(){
    ...
    public function Save(){
        ...
        $cid    =      'C'.rand(001,1000000);        
        $batchno    =   rand(0000005,10000000);        
        ...
        $this->form_validation->set_rules('cphone', 'phone', 'trim|required|xss_clean');
...
/*        echo date('F Y',strtotime('-1 month')).' : '.$total.' '."Target:".$targetbalance->target_amount.'   '."Discount:".$targetbalance->target_discount.'%';*/
                              echo"<div class='dues'>";
            if($dues->total_due > 0){ 
                                  echo"<h4 class='previous-due-header'><span class='previous-dues'>Dette = </span> $dues->total_due FCFA</h4>";
            }
                              echo"</div>
                               <div class='borderc'>
                                   <h5 class='discount-info'>Total Facture du Mois:</br> <span>";echo date('F Y').' : '. "</span> $total FCFA</h5>
                                  <!-- <h5 class='discount-info'><span>Montant Cible:</span>$targetbalance->target_amount FCFA</h5>--!>
                                   <h5 class='discount-info'><span>Réduction :</span>";if($targetbalance->target_amount < $total){ echo $targetbalance->target_discount; } else {
 ...
 //delete 
    public function Delete(){
       $id=$this->input->get('id');
       $data['customer']=$this->customer_model->DeleteCustomerID($id); 
       echo "Successfully Deleted";
       redirect(base_url().'Customer/View','refresh');
    }
     //delete 
    public function DeleteLedger(){
       $id=$this->input->get('id');
       $data['customer_ledger']=$this->customer_model->DeleteCustomerLedgerID($id); 
       echo "Successfully Deleted";
       redirect(base_url().'Customer/Balance','refresh');
    }
