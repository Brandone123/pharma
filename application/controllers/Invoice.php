...
class Invoice extends CI_Controller {
             ...
             $smrp = ($mrp);
                echo "<tr class='premove'>
                                  <td><input type='hidden' class='pid' value='$pid' name='pid[]'><input type='text' value='$product->product_name($product->strength)' readonly></td>
                                   <td> <input type='text' class='smrp' value='$smrp FCFA' name='smrp[]' readonly></td>
                                <td><input type='text' class='qty' value='$qty' name='qty[]' readonly></td>

                                  <td><input type='hidden' class='total' value='$total' name='total[]' readonly><input type='text' class='totall' value='$totall FCFA' name='totall[]' readonly></td>
        ...
        $product = $this->invoice_model->SpecificMedicine($pid);       
                ...
                echo "          <tr class='premove'>
                                ...
                                <td><input type='text' class='qty' value='$qty' name='qty[]' readonly></td>
                                  ...
                                  <input type='text' class='totall' value='$totall FCFA' name='totall[]' readonly></td>
                                  <td class='text-nowrap'>
    ...
    public function Save(){
        $salesid      =   'S'.rand(0001,10000000);
        $customer   =   $this->input->post('customerid');
        $invoice    =   rand(0000000001,50000000000);
        date_default_timezone_set("Africa/Douala");
    ...
    public function Hold_Pos(){
        $salesid    =   'S'.rand(0002,10000000);
        $customer   =   $this->input->post('cid');
        $invoice    =   rand(00000001,50000000);
        date_default_timezone_set("Africa/Douala");
        $entrydate  =   strtotime(date("Y/m/d"));
        $monthyear  =   date('Y-m');
         $time = strtotime(date('Y-m-d H:i:s'));
    ...
    public function Save_Pos_invoice(){
        ...
        $this->load->library('form_validation');
        $this->form_validation->set_error_delimiters();
        $this->form_validation->set_rules('cid', 'customer', 'trim|xss_clean');
        $this->form_validation->set_rules('pay', 'Pay Amount', 'trim|required|xss_clean');
        if($this->form_validation->run() == FALSE){
		    //$response['status'] = 'error';
        ...
        if(empty($customer)){
            ...
            if($this->db->affected_rows()){
                ...
                $paid = $grandamount - $duea;
        echo "<div class='card-body pos_receipt' style='text-align:center;'>
                                <div class='receipt_header'>
                                  ...
                                  <p class='company-info' style='padding-bottom:5px;margin-top:-10px;'>
                                    <span style='text-align:center;'>
                                     <img src='http://localhost/pharma/assets/images/$settings->sitelogo' alt='Logo' class='img-responsive text-center' style='width:120px;height:auto;'>
                                    </span>
                                    <span style='text-align:center;font-size: 12px;font-weight: 600;color: #000;line-height:15px;'> $settings->name</span>
                                  ...
                                  <p class='customer-details;margin-bottom:5px;'>
                                    ...
                                    <span style='float:right;right;font-size: 12px;font-weight: 600;color: #000'>Facture N°$invoice</span><br>
                                ...
                                <div class='receipt_body'>
                                  ...
                                  <thead>
                                    <th style='right;font-size: 13px;font-weight: 600;color: #000'>N°D</th>
                                    <th style='right;font-size: 13px;font-weight: 600;color: #000'>Produit</th>
                                    <th style='right;font-size: 13px;font-weight: 600;color: #000'>Qté</th>
                                     <th style='right;font-size: 13px;font-weight: 600;color: #000'>Prix HT</th>
                                    <th style='text-align:right;right;font-size: 13px;font-weight: 600;color: #000'>Totaux</th>
                                  </thead> 
                                  <tbody>";
                $id = 0;
        foreach($_POST['qty'] as $row=>$name):
            ...
            <td style='right;font-size: 12px;font-weight: 600;color: #000'>";echo $id; echo"</td>
              ...
              <td style='right;font-size: 12px;font-weight: 600;color: #000'>$qty</td>
              <td style='right;font-size: 12px;font-weight: 600;color: #000'>$mresult->mrp FCFA</td>
              <td style='right;font-size: 12px;font-weight: 600;color: #000'>$total FCFA</td>              
            </tr>";
                }
                endforeach;
          echo "</tbody></table>
                              ...
                              <table style='font-size:8px'>
                                <tr>
                                  ...
                                  <td colspan='4'></td>
                                  <td style='right;font-size: 12px;font-weight: 600;color: #000'>Payé: $paid FCFA</td>
                                </tr>
                                <tr>
                                    <td colspan='4'></td>
                                  <td style='right;font-size: 12px;font-weight: 600;color: #000'>Monnaie: $change FCFA</td>
                                </tr>
                                <tr>
                                  <td colspan='4'></td>
                                  <td style='right;font-size: 12px;font-weight: 600;color: #000'>Reste à Payer: $duea FCFA</td>
                                </tr>
                              </table>
                            </div>
                            <div class='receipt_footer'>
                              <span style='text-align:center' style='right;font-size: 15px;font-weight: 600;color: #000'>$settings->description</span>
                            </div>                          
                          </div>";
         }
    ...
    public function Delete(){
       ...
       $data['invoice']=$this->invoice_model->DeleteInvoiceID($id); 
       echo "Successfully Deleted";
       redirect(base_url().'Invoice/manage_Invoice','refresh');
    }
