...
class Login extends CI_Controller {
	...
	$this->form_validation->set_rules('em_name', 'Name', 'trim|xss_clean|required|min_length[5]');
	...
	if($this->form_validation->run() == FALSE){
        ...
        //Validating login
        $login_status = $this->validate_login($name, $password, $time);
    ...
    function validate_login($name = '', $password = '',$time='') {

        $credential = array('em_name' => $name, 'password' => $password,'status' => 'ACTIVE');
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->session->set_userdata('em_name', $row->em_name);
...
function logout(){
	...
	if($password == $confirm){
		...
		$this->form_validation->set_rules('em_name','Name','trim|required|min_length[5]|max_length[100]|xss_clean');
