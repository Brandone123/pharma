...
class Employee extends CI_Controller {
    ...
    public function Save(){
        ...
        $this->form_validation->set_rules('emphone', 'phone', 'trim|required|min_length[9]|max_length[13]|xss_clean');
        #$this->form_validation->set_rules('ememail', 'email', 'trim|min_length[7]|max_length[150]|xss_clean');
        #$this->form_validation->set_rules('emaddress', 'address', 'trim|min_length[5]|max_length[256]|xss_clean');
        if($this->form_validation->run() == FALSE){
		    $response['status'] = 'error';
            $response['message'] = validation_errors();
            $this->output->set_output(json_encode($response));
        } else {
           if($this->login_model->Does_email_exists($email == FALSE) && $password == $confirm){
            $response['status'] = 'error';
    ...
    public function Delete(){
       $id=$this->input->get('id');
       $data['user']=$this->user_model->DeleteEmployeeID($id); 
       echo "Successfully Deleted";
       redirect(base_url().'Employee/view','refresh');
    }
